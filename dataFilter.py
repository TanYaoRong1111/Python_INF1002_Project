...
def dataFiltering(res):
        ...
        df = df[~df["brandName"].str.contains("No Brand", case=True, na=False)]
...
def data_processing(df):
    ...
    filtered_df = df[['name', 'price', 'brandName', 'ratingScore', 'sellerName', 'itemSoldCntShow', 'review']]
    ...
    # Round to 0 decimal places for raiting
    filtered_df["Number of Sales"] = filtered_df["Number of Sales"].round(0)

    return filtered_df

def create_csv():
    res = webScrapingLazadaSingapore("Phone", page=50)

    df = dataFiltering(res)

    df = data_processing(df)

    df.to_csv("Singapore_Phone.csv")
